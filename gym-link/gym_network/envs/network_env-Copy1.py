...
class NetworkEnv(gym.Env):
    ...
    def __init__(self):
        ...
        Added Code
        """
        self.max_read_spead = 4 * 10 * 1024 * 1024 * 1024 / 8 # 4 * 10 Gigabits - all rates are in B/s
        self.max_rate = 0
    ...
    Added Code
    """
    def get_max_read_spead(self):
        maxrs = 0
        for i in range(4):
            maxrs = self.max_read_speed + maxrs
    ...
    def step(self, action):
        ...
        # find current read speed
        self.current_read_speed += int(np.random.standard_normal() * 8 * 1024 * 1024)
        #fluctuates the read speed: not every read speed is the same size.
        if self.current_read_speed > self.read_speed_max:
            self.current_read_speed = self.read_speed_max
        if self.current_read_speed < self.read_speed_min:
            self.current_read_speed = self.read_speed_min
